...
const CartCoupon = () => {
  ...
  const applyCoupon = async () => {
    if (couponCode.trim().length === 0) {
      return message.error("Please enter a valid coupon code.");
    }
    try {
      const res = await fetch(`${apiUrl}/api/coupons/code/${couponCode}`);
      if (!res.ok) {
        return message.warning("Coupon code not found");
      }
      ...
      message.success(`${couponCode} Coupone applied successfully!`);
    } catch (error) {
      console.log(error);
    }
  };
